...
interface GardenGridProps {
  ...
  positionLocks: Record<number, ActionLock>;
}

export function GardenGrid({ plants, onPlantAction, onPlantSeed, positionLocks }: GardenGridProps) {
    ...
    <div className="bg-green-50 rounded-xl p-3 sm:p-6 border-2 border-green-200">
      <div className="grid grid-cols-3 gap-2 sm:gap-4 max-w-xs sm:max-w-md mx-auto">
        {gridPositions.map((position) => {
          ...
          const lock = positionLocks[position];
          return (
            <PlantCell
              key={position}
              position={position}
