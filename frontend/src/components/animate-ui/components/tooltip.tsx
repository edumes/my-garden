...
function TooltipProvider({
  ...
  const timeoutRef = React.useRef<number>(null) as any;
...
function TooltipArrow({ side }: TooltipArrowProps) {
  return (
    <div
      className={cn(
        'absolute bg-popover dark:bg-popover z-50 size-2.5 rotate-45 rounded-[2px]',
        ...
        <TooltipPortal>
          ...
          >
            <motion.div
              ...
              transition={transition}
              className="relative rounded-md bg-popover dark:bg-popover border border-border px-3 py-1.5 text-sm text-popover-foreground shadow-lg w-fit text-balance"
            >
