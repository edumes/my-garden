package audit

import (
	"bytes"
	"io"

	"github.com/gin-gonic/gin"
	"github.com/google/uuid"
)

type AuditMiddleware struct {
	auditService *AuditService
}

func NewAuditMiddleware(auditService *AuditService) *AuditMiddleware {
	return &AuditMiddleware{
		auditService: auditService,
	}
}

type responseWriter struct {
	gin.ResponseWriter
	body *bytes.Buffer
}

func (w responseWriter) Write(b []byte) (int, error) {
	w.body.Write(b)
	return w.ResponseWriter.Write(b)
}

func (m *AuditMiddleware) AuditLog() gin.HandlerFunc {
	return func(c *gin.Context) {
		var bodyBytes []byte
		if c.Request.Body != nil {
			bodyBytes, _ = io.ReadAll(c.Request.Body)
			c.Request.Body = io.NopCloser(bytes.NewBuffer(bodyBytes))
		}

		writer := &responseWriter{
			ResponseWriter: c.Writer,
			body:           bytes.NewBufferString(""),
		}
		c.Writer = writer

		c.Next()

		statusCode := c.Writer.Status()

		var userID *uuid.UUID
		if userIDVal, exists := c.Get("user_id"); exists {
			if id, ok := userIDVal.(uuid.UUID); ok {
				userID = &id
			}
		}

		action := m.determineAction(c)
		if action == AuditAction("unknown") {
			return
		}
		resource := m.determineResource(c)
		resourceID := m.extractResourceID(c)
		status := m.determineStatus(statusCode)

		m.auditService.LogEvent(c.Request.Context(), AuditEvent{
			UserID:     userID,
			Action:     action,
			Resource:   resource,
			ResourceID: resourceID,
			Status:     status,
			Request:    c.Request,
		})
	}
}

func (m *AuditMiddleware) determineAction(c *gin.Context) AuditAction {
	path := c.Request.URL.Path
	method := c.Request.Method

	switch {
	case path == "/api/v1/gardens" && method == "POST":
		return AuditActionGardenCreate
	case path == "/api/v1/gardens" && method == "GET":
		return AuditActionGardenView
	case path == "/api/v1/gardens" && method == "DELETE":
		return AuditActionGardenDelete
	case path == "/api/v1/gardens" && method == "GET":
		return AuditActionGardenView
	case path == "/api/v1/gardens" && method == "POST":
		return AuditActionPlantSeed
	case path == "/api/v1/gardens" && method == "POST":
		return AuditActionPlantHarvest
	case path == "/api/v1/gardens" && method == "DELETE":
		return AuditActionPlantRemove
	case path == "/api/v1/garden-shares/access-links" && method == "POST":
		return AuditActionAccessLinkCreate
	case path == "/api/v1/garden-shares/join" && method == "POST":
		return AuditActionGardenJoin
	case path == "/api/v1/garden-shares" && method == "PUT":
		return AuditActionSharePermission
	case path == "/api/v1/garden-shares" && method == "DELETE":
		return AuditActionShareRemove
	case path == "/api/v1/store/buy" && method == "POST":
		return AuditActionSeedPurchase
	default:
		return AuditAction("unknown")
	}
}

func (m *AuditMiddleware) determineResource(c *gin.Context) AuditResource {
	path := c.Request.URL.Path

	switch {
	case path == "/api/v1/auth/register" || path == "/api/v1/auth/login" || path == "/api/v1/auth/logout" || path == "/api/v1/auth/refresh":
		return AuditResourceUser
	case path == "/api/v1/users/profile":
		return AuditResourceUser
	case path == "/api/v1/gardens":
		return AuditResourceGarden
	case path == "/api/v1/plants":
		return AuditResourcePlant
	case path == "/api/v1/garden-shares":
		return AuditResourceShare
	case path == "/api/v1/store":
		return AuditResourceStore
	default:
		return AuditResourceSystem
	}
}

func (m *AuditMiddleware) extractResourceID(c *gin.Context) *uuid.UUID {
	if id := c.Param("id"); id != "" {
		if parsed, err := uuid.Parse(id); err == nil {
			return &parsed
		}
	}
	if id := c.Param("garden_id"); id != "" {
		if parsed, err := uuid.Parse(id); err == nil {
			return &parsed
		}
	}
	if id := c.Param("plantId"); id != "" {
		if parsed, err := uuid.Parse(id); err == nil {
			return &parsed
		}
	}
	if id := c.Param("user_id"); id != "" {
		if parsed, err := uuid.Parse(id); err == nil {
			return &parsed
		}
	}
	return nil
}

func (m *AuditMiddleware) determineStatus(statusCode int) AuditStatus {
	switch {
	case statusCode >= 200 && statusCode < 300:
		return AuditStatusSuccess
	case statusCode >= 400 && statusCode < 500:
		return AuditStatusFailure
	case statusCode >= 500:
		return AuditStatusError
	default:
		return AuditStatusFailure
	}
}
